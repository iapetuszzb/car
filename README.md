...
| 目录 | 控制器 | 用途 |
| --- | --- | --- |
| 仓库根目录 | TI MSPM0G3507 | 当前两轮小车巡线、蓝牙闭环运动、编码器里程计和 MPU6500 姿态控制 |
| `K230D/steel_ball/` | 正点原子 K230D BOX | 当前钢球识别、最近目标选择、短时漏检坐标保持和 PORT2 坐标发送 |
| `STM32/k230_v1.0.0/` | C06B / STM32F103 | 单轴钢球平衡控制实验，包含滤波、PD、限幅、STEP/DIR 规划和测试 |

K230D 当前上板脚本及 UART 接线见 `K230D/README.md`。STM32/C06B 工程的硬件假设、编译方式和安全限制见 `STM32/k230_v1.0.0/README.md`。

需要注意，两套视觉通信协议目前不同：当前 K230D 上板脚本发送 `X{X差}Y{Y差}Z{有效}E`，STM32/C06B 实验工程默认接收单个有符号 X 像素误差和换行。两者不能未经适配直接混接。
...
Drivers/
  ...
  Uart/                  蓝牙协议和闭环运动控制
K230D/
  steel_ball/            K230D 当前钢球识别脚本和 best1.kmodel
  tools/                 PORT1/PORT2 UART 回环测试
STM32/
  k230_v1.0.0/           C06B/STM32F103 单轴钢球平衡控制实验工程
